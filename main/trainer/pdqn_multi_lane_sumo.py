...
wandb.init(project="",entity="",name="machine 1660",config=config)
...
from Sumo.sumo_integration.bridge_helper import BridgeHelper  # pylint: disable=wrong-import-position
from Sumo.sumo_integration.carla_simulation import CarlaSimulation  # pylint: disable=wrong-import-position
from Sumo.sumo_integration.constants import INVALID_ACTOR_ID  # pylint: disable=wrong-import-position
from Sumo.sumo_integration.sumo_simulation import SumoSimulation  # pylint: disable=wrong-import-position
from Sumo.run_synchronization import SimulationSynchronization
import traci
...
def main():
    ...
    env = CarlaEnv(args)
    print("sumo_gui:  ", args.sumo_gui)
    sumo_simulation = SumoSimulation(args.sumo_cfg_file, 1.0 / args.fps, args.sumo_host,
    ...
    for run in [base_name]:
        agent = P_DQN(s_dim, a_dim, a_bound, GAMMA, TAU, SIGMA_STEER, SIGMA, SIGMA_ACC, THETA, EPSILON, BUFFER_SIZE, BATCH_SIZE, LR_ACTOR,
                     LR_CRITIC, clip_grad, zero_index_gradients, inverting_gradients,PER_FLAG, DEVICE, args)
        ...
        try:
            for i in range(10):
                with tqdm(total=TOTAL_EPISODE // 10, desc="Iteration %d" % i) as pbar:
                    for i_episode in range(TOTAL_EPISODE // 10):
                        ...
                        acc_list = []
                        reward_list = []
                        synchronization = SimulationSynchronization(sumo_simulation, env, args.tls_manager,
                                                                    args.sync_vehicle_color, args.sync_vehicle_lights)
                        while not done and not truncated:
                            action, action_param, all_action_param = agent.take_action(state)
                            next_state, reward, truncated, done, info, sk = env.step(action, action_param)
                            synchronization.tick()
                            e_c, pos, vel, acc = synchronization.sumo.tick()
                        ...
                        np.save(f"{SAVE_PATH}/acc.npy", np.array(acc_list))
                        np.save(f"{SAVE_PATH}/vel.npy", np.array(vel_list))
                        np.save(f"{SAVE_PATH}/e_c.npy", np.array(e_c_list))
                        np.save(f"{SAVE_PATH}/pos.npy", np.array(pos_list))
                        np.save(f"{SAVE_PATH}/reward.npy", np.array(reward_list))
                        # record episode results
                        if env.RL_switch:
                            # wandb.log({
                            #     'Total_Reward': score,
                            #     'Avg_Reward': score/(env.time_step+1),
                            #     'Time_Steps': env.time_step,
                            #     'TTC': ttc/(env.time_step+1),
                            #     'Efficiency': efficiency/(env.time_step+1),
                            #     'Comfort': comfort/(env.time_step+1),
                            #     'Lcen': lcen/(env.time_step+1),
                            #     'Yaw': yaw/(env.time_step+1),
                            #     'Impact': impact/(env.time_step+1),
                            #     'Lane_change_reward': lane_change_reward/(env.time_step+1)
                            # })
            ...
            wandb.finish()
            # for value in e_c_list:
            #     print(value)
            # for value in pos_list:
            #     print(value)
            # for value in vel_list:
            #     print(vel)
        ...
        if control_state:
            ...
            if truncated:
                agent.store_transition(state, action, saved_action_param, reward, next_state,
                                    truncated, done, info, action, saved_action_param)
            else:
                ...
                if len(impact_deque) == 2:
                    ...
                    agent.store_transition(experience[0], experience[1], experience[2],
                                            ...
                                            experience[6], experience[7], action, saved_action_param)
