...
class PolicyNet_m(torch.nn.Module):
    def __init__(self, state_dim, action_parameter_size, action_bound, train=True) -> None:
        # the action bound and state_dim here are dicts
        super().__init__()
        self.state_dim = state_dim
        ...
        self.fc_out = nn.Linear(64, self.action_parameter_size)
        # torch.nn.init.normal_(self.fc1_1.weight.data,0,0.01)
        # torch.nn.init.normal_(self.fc1_2.weight.data,0,0.01)
        # torch.nn.init.normal_(self.fc_out.weight.data,0,0.01)
        # torch.nn.init.normal_(self.fc_out.weight.data,0,0.01)
    ...
    def forward(self, state):
        ...
        action = torch.tanh(self.fc_out(x2))
        # steer,throttle_brake=torch.split(out,split_size_or_sections=[1,1],dim=1)
        # steer=steer.clone()
        # throttle_brake=throttle_brake.clone()
        # steer*=self.action_bound['steer']
...
class QValueNet_m(torch.nn.Module):
    def __init__(self, state_dim, action_param_dim, num_actions) -> None:
        # parameter state_dim here is a dict
        super().__init__()
        self.state_dim = state_dim
        ...
        self.fc1 = nn.Linear(self.state_dim, 64)
        self.fc2 = nn.Linear(64, 64)
        self.action_encoder = nn.Linear(self.action_param_dim, 64)
    ...
    def forward(self, state, action):
        ...
        x2 = F.relu(self.fc2(x1))
        action_enc = self.action_encoder(action)
        state_ = torch.cat((x2, action_enc), dim=1)
        out = self.fc_out(state_)
        return out
...
class P_DQN:
    def __init__(self, state_dim, action_dim, action_bound, gamma, tau, sigma, sigma_steer, sigma_acc, theta, epsilon,
                 buffer_size, batch_size, actor_lr, critic_lr, clip_grad, zero_index_gradients, inverting_gradients, per_flag,device, args) -> None:
        self.learn_time = 0
        self.replace_a = 0
        self.replace_c = 0
        ...
        self.train = True
        self.actor = PolicyNet_m(self.s_dim, self.action_parameter_size, self.a_bound).to(self.device)
        self.actor_target = PolicyNet_m(self.s_dim, self.action_parameter_size, self.a_bound).to(self.device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        if not self.td3:
            self.critic = QValueNet_m(self.s_dim, self.action_parameter_size, self.num_actions).to(self.device)
            self.critic_target = QValueNet_m(self.s_dim, self.action_parameter_size, self.num_actions).to(self.device)
        else:
    ...
    def learn(self):
        ...
        self.replace_c += 1
        if not self.per_flag:
            b_s, b_a, b_a_param, b_r, b_ns, b_t, b_d, b_a_, b_a_param_ = self.replay_buffer.sample(self.batch_size)
        else:
        ...
        batch_a_ = torch.tensor(b_a_, dtype=torch.int64).view((self.batch_size, -1)).to(self.device)
        batch_a_param_ = torch.tensor(b_a_param_, dtype=torch.float32).view((self.batch_size, -1)).to(self.device)
        with torch.no_grad():
            ...
            penality = ((action_param_target-batch_a_param_)**2).sum(-1)
            q_targets = batch_r + self.gamma * (q_prime-args.beta*penality) * (1 - batch_t)
        if not self.td3:
    ...
    def store_transition(self, state, action, action_param, reward, next_state, truncated, done, info, next_action, next_action_paaram):  
        ...
        if not self.per_flag:
            if action == 0 or action == 2:
                self.replay_buffer.add((state, action, action_param, reward, next_state, truncated, done, next_action, next_action_paaram),False)
            self.replay_buffer.add((state, action, action_param, reward, next_state, truncated, done, next_action, next_action_paaram),True)
        else:
